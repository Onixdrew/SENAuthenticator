...
const Reports = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      <h1 className="text-3xl font-bold mb-4">Página de Reportes</h1>
      ...
      <section className="mb-8">
        ...
        <div className="flex flex-wrap gap-4">
          {['Fichas', 'Documentos', 'Tiempos', 'Graficas'].map((item) => (
            <div key={item} className="flex-1 min-w-[150px] bg-gray-200 p-4 rounded-lg text-center hover:bg-gray-300 transition duration-300">
              {item}
            </div>
          ))}
        </div>
          ...
          <table className="min-w-full bg-white border border-gray-200 rounded-lg shadow-md">
            <thead className="bg-gray-200">
              <tr>
                ...
                <th className="py-2 px-4 border-b">Llegó</th>
            ...
            <tbody>
              {data.map((item) => (
                <tr key={item.id} className="hover:bg-gray-100">
                  ...
                  <td className="py-2 px-4 border-b text-center">{item.apellido}</td>
                  <td className="py-2 px-4 border-b text-center">{item.tipoDocumento}</td>
                  <td className="py-2 px-4 border-b text-center">{item.numero}</td>
                  <td className="py-2 px-4 border-b text-center">{item.llego ? 'Sí' : 'No'}</td>
                  <td className="py-2 px-4 border-b text-center">{item.horaEntrada}</td>
                </tr>
              ))}
            </tbody>
          </table>
